# implement the Evaluating main function for the C-Decision scheme with the Real Feedback
...
def main():
    ...
    for BS_loop in range(num_BS_Out):
        ...
        FB_str = '>>>>>>>>>Testing FB = ' + str(curr_FB) + ' at different random seeds<<<<<<<<<'
        print(FB_str)
        # start training
...
def load_trained_model(Env, curr_RL_Config):
    # load the trained C-Decision model for evaluation
...
def run_test(curr_RL_Config, BS_Agent, test_seed_sequence, Fixed_Epsilon, num_evaluate_trials):
    # run the test according to current settings for evaluation via the trained model
    ...
    # for tracking of the evaluation
    print("-----Current Evaluation Parameters Settings are: ")
    ...
    if Opt_Flag:
        ...
        print('To Run C-Decision TEST with Optimal Scheme!')
        ...
        print('To Run C-Decision Test without Optimal Scheme!')
        ...
        if Curr_OS == 'nt':
            Fig_Dir = folder
        # compare mean return with errors under C-Decision scheme and Random Action scheme
        ...
        x_max = Num_Run_Episodes * 5
        my_x_ticks = np.arange(0, x_max, 5)
        label_str = 'C-Decision with ' + '$\epsilon = $' + str(Fixed_Epsilon)
        x = my_x_ticks
