...
class Environ:
    ...
    def add_new_vehicles_by_number(self, n):
        for i in range(n):
            ...
            start_direction = 'r'
            # start_velocity  randomly choose from 10 Km/hour to 15 Km/hour
            self.add_new_vehicles(start_position, start_direction, random.randint(10, 15))
    ...
    def renew_neighbor(self):
        ...
        # ===========================================
        # initialize vehicles.neighbor and actions
        ...
        # compute distances between any two vehicles
        Distance = abs(z.T-z)
        for i in range(len(self.vehicles)):
            sort_idx = np.argsort(Distance[:, i])  # sort vehicle according to their distances to the target vehicle
            # choose the n_Neighbor vehicles whose distances is close to the target vehicle
            for j in range(self.n_Neighbor):
                self.vehicles[i].neighbors.append(sort_idx[j+1])
            # choose the nearest vehicle as the destination of the target vehicle
    ...
    def compute_reward_with_channel_selection(self, actions_ch_sel):
        ...
        # ===================================================
        # Input: actions_ch_sel: [size: N_vehicles x N_neighbors]
        actions = actions_ch_sel
        # power selection index matrix
        ...
        for i in range(self.n_RB):
            # find which D2Ds transmit on the i-th RB
            indexes = np.argwhere(actions == i)
            for j in range(len(indexes)):
                # find the receiver of the j-th D2D who transmits on the i-th RB
                receiver_j = self.vehicles[indexes[j, 0]].destinations[indexes[j, 1]]
                # compute V2V RX signal power
                ...
                # calculate the interference of V2I links to V2V links
                if V2I_Flag:
                    if i < self.n_Veh:
                        # V2I links interference to V2V links on the i-th RB
    ...
    def Compute_Interference(self, actions):
        ...
        # ====================================================
        # V2V_Interference： size [ Num_D2D x Num_Neighbor x Num_RB]
        ...
        if len(actions.shape) == 2:
            ...
            # the interference of V2V links to V2V links
            for i in range(len(self.vehicles)):
