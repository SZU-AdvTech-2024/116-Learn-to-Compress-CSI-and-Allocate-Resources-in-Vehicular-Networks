# implement the Train main function for the C-Decision scheme with the Real Feedback
...
def main():
    ...
    for train_loop in range(num_train_settings):
        ...
        [Train_Loss, Reward_Per_Train_Step, Reward_Per_Episode,
         Train_Q_mean, Train_Q_max_mean, Orig_Train_Q_mean, Orig_Train_Q_max_mean] = run_train(Env, curr_RL_Config)
...
def save_train_results(Train_Loss, Reward_Per_Train_Step, Reward_Per_Episode,
    ...
    # compute the original Q value
    Orig_Train_Q_mean_per_Episode = np.sum(Orig_Train_Q_mean, axis=1) / Num_Train_Step
    Orig_Train_Q_max_mean_per_Episode = np.sum(Orig_Train_Q_max_mean, axis=1) / Num_Train_Step
    ...
    # plot Reward per training Episode
    x = range(Num_Episodes)
    ...
    # save the figure
    Fig_Name = 'Train-Target-Value-plot' + '-Episode-' + str(Num_Episodes) + '-Step-' + str(Num_Train_Step) \
               + '-Batch-' + str(Batch_Size) + '.png'
    Fig_Para = Fig_Dir + Fig_Name
    plt.savefig(Fig_Para, dpi=600)
    # use for latex
    Fig_Name1 = 'Train-Target-Value-plot' + '-Episode-' + str(Num_Episodes) + '-Step-' + str(Num_Train_Step) \
                + '-Batch-' + str(Batch_Size) + '.eps'
    Fig_Para1 = Fig_Dir + Fig_Name1
    plt.savefig(Fig_Para1, dpi=600)

    # plot the Q mean and Q max mean results of Original Q function
    x = range(Num_Episodes)
    y = Orig_Train_Q_mean_per_Episode
    y1 = Orig_Train_Q_max_mean_per_Episode
    plt.figure()
